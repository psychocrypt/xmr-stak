...
// cryptonight_monero_v8
#if(ALGO==11)
			{
				...
				SCRATCHPAD_CHUNK(1) = as_uint4(chunk3 + ((ulong2 *)(&b_x1))[0]);
				SCRATCHPAD_CHUNK(2) = as_uint4(chunk1 + ((ulong2 *)&b_x0)[0]);
...
#	if(ALGO == 7)
			uint index = ((b_x0.s2 >> 27) & 12) | ((b_x0.s2 >> 23) & 2);
#	else
			uint index = ((b_x0.s2 >> 26) & 12) | ((b_x0.s2 >> 23) & 2);
#	endif
			b_x0.s2 ^= ((table >> index) & 0x30U) << 24;
			SCRATCHPAD_CHUNK(0) = b_x0;
			idx0 = c[0] & MASK;
// cryptonight_monero_v8
#elif(ALGO==11)
			SCRATCHPAD_CHUNK(0) = b_x0 ^ ((uint4 *)c)[0];
#	ifdef __NV_CL_C_VERSION
			// flush shuffled data
