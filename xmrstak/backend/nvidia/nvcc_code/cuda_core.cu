...
{
	int chain_height = 100;

	if((miner_algo == cryptonight_r_wow) || (miner_algo == cryptonight_r)  || (miner_algo == cryptonight_r_64))
	{
		if(ctx->kernel_height < chain_height)
		{
			 if(ctx->module)
	...
	static const cuda_hash_fn func_table[] = {
		...
		cryptonight_core_gpu_hash<cryptonight_r_64, 0>,
		cryptonight_core_gpu_hash<cryptonight_r_64, 1>
	};
