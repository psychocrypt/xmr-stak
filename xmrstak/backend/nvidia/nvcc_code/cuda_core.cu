...
template<xmrstak_algo_id ALGO, uint32_t MEM_MODE>
void cryptonight_core_gpu_hash(nvid_ctx* ctx, uint32_t nonce, xmrstak_algo algo)
{
	const uint32_t MASK = algo.Mask();
	const uint32_t ITERATIONS = algo.Iter()/4u;
...
template<xmrstak_algo_id ALGO, uint32_t MEM_MODE>
void cryptonight_core_gpu_hash_gpu(nvid_ctx* ctx, uint32_t nonce, xmrstak_algo algo)
{
	const uint32_t MASK = algo.Mask();
...
{
	typedef void (*cuda_hash_fn)(nvid_ctx* ctx, uint32_t nonce, xmrstak_algo algo);
