...
static FORCEINLINE void check_data(size_t* data_index, const size_t bytes_needed, uint8_t* data, const size_t data_size)
{
	if (*data_index + bytes_needed > data_size)
	{
		blake256_hash(data, data, data_size);
		*data_index = 0;
	}
}
...
static int v4_random_math_init(struct V4_Instruction* code, const uint64_t height)
{
	...
	if ((ALGO == cryptonight_r) || (ALGO == cryptonight_r_64))
	{
		data[20] = -38;
	}
